...
import blazo.dev.validators.Validations;
...
public class AppManager {
    ...
    private static void addPerson(String type) {
        ...
        String name = Validations.askForNonEmptyString(scanner, "Please enter the " + type + "'s name: ");
        int age = Validations.askForPositiveInt(scanner, "Please enter the " + type + "'s age: ");
        String email = Validations.askForNonEmptyString(scanner, "Please enter the " + type + "'s email: ");
        ArrayList<Course> personCourses = addCourses(type);

        if (type.equals("student")) {
            int grade = Validations.askForPositiveInt(scanner, "Please enter the student's grade: ");
            Student newStudent = new Student(name, (byte) age, email, grade, personCourses);
            students.add(newStudent);
            System.out.println("\n🎉 Student " + name + " has been successfully added!");
        } else if (type.equals("teacher")) {
            String subject = Validations.askForNonEmptyString(scanner, "Please enter the teacher's subject: ");
            Teacher newTeacher = new Teacher(name, (byte) age, email, subject, personCourses);
    ...
    private static ArrayList<Course> addCourses(String type) {
        ...
        int numberOfCourses = Validations.askForPositiveInt(scanner, "How many courses is this " + type + " involved with? ");
        ...
        for (int i = 0; i < numberOfCourses; i++) {
            String courseName = Validations.askForNonEmptyString(scanner, "Enter the name of course " + (i + 1) + ": ");
